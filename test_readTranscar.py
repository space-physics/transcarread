...
from transcarread.readTranscar import calcVERtc,SimpleSim,plotPeigen
...
if __name__ == '__main__':
    ...
    p.add_argument('--path',help='root path that beam directories live in',default='~/data/transcar')
    p.add_argument('-e','--energy',help='energy of this beam [eV]',default=2300.5,type=float)
    p.add_argument('-t','--treq',help='date/time  YYYY-MM-DDTHH-MM-SS',default='2013-04-10T0:0:0Z')
...
#%% run sim
    if p.profile:
        ...
        Peigen, EKpcolor, Plambda = calcVERtc(infile,p.path,p.energy,parse(p.treq),sim)
        plotPeigen(Peigen)
